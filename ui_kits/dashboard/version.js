...
window.NT_CHANGELOG = [
  {
    ...
    notes: [
      ...
      "The single “Portfolio” page has grown into a full second world called Swings, with its own Dashboard, Alerts, Trades and Strategies pages — the same shape as the options side, so nothing has to be learned twice. A small Options / Swings switch appears at the top of the menu as soon as you have both (with only one, there's nothing to switch, so it stays hidden). The Swings dashboard opens with six headline numbers — your account return, this year's return, net profit, win rate, average return per trade and how you're doing against the S&P 500 — followed by your current holdings with the sheet's target weight and how long you've held each one. Alerts is the running list of everything the publisher changes (an advice flip, a weight change, an exit, a brand-new holding), with a live/lagging/stale badge for how recently the sheet was checked; anything that's only tracked and never ordered (physical metal) is greyed out and says so. Activity stays shared by both worlds, and broker/source setup stays in Settings.",
    ],
  },
  {
