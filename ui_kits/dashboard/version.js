...
window.NT_CHANGELOG = [
  {
    ...
    notes: [
      ...
      "BETA is now a pill, not a word: it sits next to Crypto in the menu, in the top-bar panels and on the crypto page heads. The crypto pages use the same cards, tables and spacing as Swings \u2014 the first version was visibly off-grid.",
      "Sentiment page, second pass: each video is now a score tile (colour is the mood, the number is how strong), a six-word title with a full one-sentence summary, the date, and a playbook tile per leader \u2014 BTC and XRP \u2014 that answers the real question: what to do now, what to prepare for, and the trigger that flips it, with a conviction bar. Open a row for the video itself, playing inside Reezer, his thesis and arguments, what would invalidate them, and every call with its quote. Alts and other coins live in the opened view only; XRP counts as an altcoin, so his market-wide alts calls apply to it.",
      "Crypto has a Sentiment page: Reezer follows the Blockchain Backer YouTube channel, reads the captions of every new video and turns them into a brief \u2014 overall read with a 0\u2013100 score, a verdict on BTC, ETH and XRP every time, and every buy / sell / wait call with the exact quote it rests on, conditions included. A push arrives on your phone per video. Read-only: nothing trades on it.",
      "Settings \u2192 Brokers has an IB Gateway line, the twin of the Schwab connection line: online since when, and when IBKR's weekly re-login is due \u2014 or OFFLINE since when, in red, with the reason. A watchdog on the Gateway's Mac probes the link every minute and pushes to your phone after three misses, so an outage is seen at once instead of discovered through a wrong number (22 Aug: a 2.5-hour silent outage became a false 'order cancelled' and a frozen P&L).",
      "A swing order is now only called cancelled on evidence: IBKR's own completed-orders list, or the same absence seen on two checks at least five minutes apart. One bad reading can no longer flip the book; the notification says which evidence it was.",
      "Fixed: the Brokers card in Settings read its rows once when opened and then froze, so it could still say ERROR minutes after the sidebar had gone green. It now refreshes on the same pulse as the rest of the dashboard.",
      "Broker health is judged by one rule everywhere now \u2014 sidebar and Settings, every world: no broker, broker error (the last check failed), or not checked today. ERROR is red. A paper-only world shows a broker error too, because the check itself is broken.",
      "Fixed: Settings \u2192 Brokers said LINKED while the sidebar said broker error. The row now shows the broker's own verdict \u2014 ERROR with when the last check failed and when the last good one was \u2014 instead of calling a day-old number linked.",
      "Fixed: broker icons in Settings could stay blank for up to a minute after the page opened \u2014 the page now draws its icons as soon as its rows arrive.",
